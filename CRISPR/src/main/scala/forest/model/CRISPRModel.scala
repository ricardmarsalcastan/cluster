...
/******************************************************************************
 * CRISPRModel tries to imitate the idea of the CRISPR technique of selecting
 ...
 * @param numForests The number of forests created.
 * @param numTrees The number of trees on each RandomForest.
...
/******************************************************************************
    ...
    out += ("\nBEST TREES SELECTION MODEL\n" + forestAccuracy
      + "\n" + forestWeightedRecall
      + "\n" + forestWeightedFalsePositiveRate)
  ...
  def createForestSelectBestTrees() = {
    ...
    for (i <- 1 to numForests) {
      ...
      for (tree <- model.trees) {
        ...
        var labelAndPreds = testingData.map { point =>
          var prediction = tree.predict(point.features)
          (point.label, prediction)
        }
